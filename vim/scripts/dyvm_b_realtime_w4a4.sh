...
#SBATCH --job-name=dyvm_b_rt_w4a4
#SBATCH --output=/u/chenjoachim/log/dyvm_b_rt_w4a4_%j.out
#SBATCH --error=/u/chenjoachim/log/dyvm_b_rt_w4a4_%j.err
...
python vim/quant.py \
  ...
  --data-set IMNET \
  --qmode ptq4vm \
  --load-quant ./checkpoints/dyvm_b_quant_w4a4.pth \
  --real-gemm \
  --batch-size 32 \
  --num_workers 2
