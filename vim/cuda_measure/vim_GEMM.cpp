...
torch::Tensor vim_GEMM(torch::Tensor x, torch::Tensor w, torch::Tensor smooth_scale, torch::Tensor scale_x, torch::Tensor scale_w, torch::Tensor z, torch::Tensor w_col_sum, int H_size, int qbit){
    ...
    return vim_GEMM_cuda(x, w, smooth_scale, scale_x, scale_w, z, w_col_sum, H_size, qbit);
}
